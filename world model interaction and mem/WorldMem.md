...
```ad-note
**第一部分：Memory Bank + 检索策略。** 记忆库存所有历史帧，检索时用 **confidence 评分** 选最相关的子集。评分基于两个信号：FOV overlap（视野重叠度，衡量空间相关性）和 temporal proximity（时间接近度）。直觉很简单——和你当前看的方向视野重叠越大、时间上越近的帧，越可能包含你需要的信息。

**第二部分：Memory Attention + State-aware Embedding。** 当前生成的 latent token 做 query，检索出的记忆 token 做 key 和 value，通过 cross-attention 融合信息。关键创新是在 Q 和 K 上都加了 state-aware embedding（位姿 + 时间戳的编码），并且用了 **相对编码**（relative embedding）——把 query 帧的坐标设为原点，记忆帧的坐标表示为相对值
```
